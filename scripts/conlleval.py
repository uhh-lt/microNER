...
def write_conll_file(true_labels, pred_labels, testSentences, idx2Label, filename):
...
def write_germeval_file(true_labels, pred_labels, true_labels_2nd, pred_labels_2nd, testSentences, idx2Label, idx2Label_2nd, filename):
    f = open(filename, 'w', encoding='UTF-8')
    for i_sent, sent in enumerate(testSentences):
        for i_tok, tok in enumerate(sent):
            if tok[0] == 'PADDING_TOKEN':
            ...
            correctlabel = idx2Label[true_labels[i_sent][i_tok]]
            guessedlabel = idx2Label[pred_labels[i_sent][i_tok]]
            correctlabel_2nd = idx2Label_2nd[true_labels_2nd[i_sent][i_tok]]
            guessedlabel_2nd = idx2Label_2nd[pred_labels_2nd[i_sent][i_tok]]
            line = "\t".join([str(i_tok), tok[0], correctlabel, correctlabel_2nd, guessedlabel, guessedlabel_2nd])
            f.write(line + '\n')
        f.write('\n')
    f.close


def evaluate_conll_file(filename):
    
    true_seqs, pred_seqs = [], []
    
    with open(filename, 'r', encoding='UTF-8') as fileIterator:
